...
const ProductCard = ({ p, onSelect }) => {
  ...
  const labelText = p.source_table === 'secondhand' ? 'SECONDHAND' : 'PREMIUM JATI';
  const labelStyles = p.source_table === 'secondhand'
    ? 'bg-blue-900/40 text-[#00FFFF] border-[#00FFFF]/30' 
    : 'bg-black/40 text-[#FFD700] border-[#FFD700]/50';
  ...
  return (
    <div onClick={() => onSelect(p)} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 cursor-pointer border border-gray-100 relative flex flex-col h-fit">
      {p.is_offer && (
    ...
    <div className="relative aspect-[4/5] overflow-hidden bg-gray-200">
        ...
        <div className="absolute top-3 left-3 z-20 flex flex-col items-start gap-1.5">
          <span className="px-2.5 py-1 bg-white/90 backdrop-blur-md rounded-full text-[9px] font-bold uppercase tracking-widest text-blue-600 shadow-sm border border-white/20">
            {p.cat}
          </span>

          <span className={`px-2 py-0.5 backdrop-blur-md rounded-md text-[8px] font-black uppercase tracking-wider border shadow-sm ${labelStyles}`}>
            {labelText}
          </span>
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col">
        ...
        <p className={`text-[11px] font-bold uppercase tracking-wider mb-2 ${totalStok > 0 ? 'text-emerald-600' : 'text-red-500'}`}>
          {totalStok > 0 ? `Stok: ${totalStok} Unit` : 'Stok Habis'}
        </p>

        <div className="min-h-[25px] flex items-center mb-3">
        {p.note && (
...
const GalleryImage = ({ src, alt }) => {
  ...
  return (
    <div className="relative rounded-[2rem] overflow-hidden shadow-xl bg-gray-200 mb-6 border border-gray-100 aspect-square">
      {!loaded && (
      <div className="absolute inset-0 z-10 bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 bg-[length:200%_100%] animate-[shimmer_1.5s_infinite] opacity-50" />      )}
...
const handleTempahan = async (variant, index) => {
  ...
  return (
    <div className="bg-[#fafafa] min-h-screen font-sans text-gray-900 relative leading-relaxed tracking-tight">
      {currentScreen === 'home' ? ( 
        <>
  ...
  <span className="text-yellow-500 text-[10px] font-bold uppercase tracking-[0.3em]">
    Koleksi Eksklusif 2026
          ...
          />
              ...
              <div className={`transition-all duration-500 ease-in-out origin-top ${showCatBar ? 'opacity-100 scale-100 h-auto' : 'opacity-0 scale-95 pointer-events-none h-0 overflow-hidden'}`}>
                <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-1 shadow-md border border-gray-100">
                  <div className="grid grid-cols-5 md:grid-cols-11 gap-2 p-1.5">
        ...
        <div className="bg-white min-h-screen">
              ...
              {selectedProduct?.variants?.map((v, i) => {
                ...
                return (
                  <div key={i} className="group animate-in fade-in slide-in-from-bottom-10 duration-700">
                    ...
                    <button 
                      onClick={() => {
                        const text = `${selectedProduct.name}. ${window.location.href}`;
                        window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
                      }}
                      className="w-full mt-4 flex items-center justify-center gap-2 py-3 border-2 border-gray-100 rounded-2xl text-[10px] font-bold text-gray-500 hover:bg-gray-50 transition-all uppercase tracking-widest"
                    >
          ...
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-lg border-t border-gray-100 md:hidden z-[100] flex items-center justify-between gap-4 animate-in slide-in-from-bottom duration-500">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Harga Dari</span>
              <span className="text-lg font-black text-blue-600">RM{Number(selectedProduct.price).toLocaleString('en-MY')}</span>
            </div>
            <button 
              onClick={() => handleTempahan(selectedProduct.variants[0], 0)}
              className="bg-[#22c55e] text-white px-6 py-3 rounded-xl font-black text-[10px] tracking-widest shadow-lg active:scale-95 transition-transform flex items-center gap-2"
            >
              <Icons.MessageCircle /> CHAT SEKARANG
            </button>
          </div>
        </div>
      )}
      ...
      {currentScreen === 'home' && (
        <footer className="bg-[#0f172a] text-white pt-20 pb-32 px-6 mt-20">
      ...
      {lastScrollY > 1000 && currentScreen === 'home' && (
        <button 
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="fixed bottom-24 right-6 p-4 bg-white shadow-2xl rounded-full text-blue-600 border border-gray-100 z-[90] active:scale-90 transition-all md:bottom-10"
        >
