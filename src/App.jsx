...
// 1. Ikon SVG Custom (Termasuk kategori baru)
const Icons = {
...
        <p className={`text-[11px] font-bold uppercase tracking-wider mb-4 ${totalStok > 0 ? 'text-emerald-600' : 'text-red-500'}`}>{totalStok > 0 ? `Stok: ${totalStok} Unit` : 'Stok Habis'}</p>
...
            <p className="text-xl font-black text-gray-900"><span className="text-xs font-medium mr-1">RM</span>{Number(p.price).toLocaleString('en-MY')}</p>
...
      {currentScreen === 'home' ? ( <>
          <header className="relative h-[75vh] min-h-[600px] flex items-center justify-center text-center px-6 overflow-hidden bg-slate-900">
            ...
            <div className="absolute inset-0 overflow-hidden">
               <img src="https://images.unsplash.com/photo-1598928636135-d146006ff4be?q=80&w=2070&auto=format&fit=crop" className="w-full h-full object-cover opacity-60 scale-105 animate-[pulse_15s_ease-in-out_infinite]" alt="Hero" style={{animationDuration: '30s'}} />
            </div>
            ...
            <div className="relative z-10 max-w-5xl flex flex-col items-center">
              {/* Gold Badge */}
              <div className="inline-flex items-center gap-3 px-6 py-2 border border-yellow-600/40 rounded-full bg-black/40 backdrop-blur-md mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-500 animate-pulse"></span>
                <span className="text-yellow-500 text-[10px] font-bold uppercase tracking-[0.3em]">Koleksi Eksklusif 2026</span>
              </div>

              {/* Main Title - Mix of Serif and Sans */}
              <h1 className="text-5xl md:text-8xl font-serif text-white mb-6 tracking-tight leading-[1.1] animate-in fade-in zoom-in-95 duration-1000 delay-150">
              ...
              <div className="flex items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-300">
                ...
                <p className="text-gray-300 text-xs md:text-sm tracking-[0.2em] uppercase font-light max-w-xl leading-relaxed">
                  by ssmaju legacy
                </p>
                <div className="h-[1px] w-12 bg-white/30 hidden md:block"></div>
              </div>
...
            <div className="max-w-4xl mx-auto px-4"> {/* Mengecilkan lebar container utama */}
              <div className="relative group mb-4">
                <div className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-600 transition-colors"><Icons.Search /></div>
                <input type="text" placeholder="Cari koleksi idaman anda..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-14 pr-6 py-4 bg-white rounded-2xl shadow-lg border-none focus:ring-4 focus:ring-blue-500/10 outline-none text-gray-700 transition-all font-medium text-sm" />
              </div>

              {/* Grid Category Lebih Kecil & Kompak */}
              <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-2 shadow-sm border border-gray-100">
                <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-10 gap-1"> {/* Grid lebih rapat (gap-1) dan lebih banyak lajur */}
                  {categoryList.map(cat => (
                    <button 
                      key={cat.id} 
                      onClick={() => { setFilter(cat.id); window.scrollTo({top: 380, behavior: 'smooth'}); }}
                      className={`flex flex-col items-center justify-center gap-1 p-2 rounded-xl transition-all
                        ${filter === cat.id ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                      <div className={`scale-75 ${filter === cat.id ? 'text-white' : 'text-gray-400'}`}>
                        {cat.icon}
                      </div>
